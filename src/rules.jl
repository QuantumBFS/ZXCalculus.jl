...
function Base.match(::Rule{:i1}, zxd::ZXDiagram{T, P}) where {T, P}
    ...
    for v1 in spiders(zxd)
        if spider_type(zxd, v1) == SpiderType.Z || spider_type(zxd, v1) == SpiderType.X
            if phase(zxd, v1) == zero(PiUnit) && (degree(zxd, v1)) == 2
                push!(matches, Match{T}([v1]))
            end
        end
    end
...
function Base.match(::Rule{:lc}, zxg::ZXGraph{T, P}) where {T, P}
    ...
    for v in vs
        if spider_type(zxg, v) == SpiderType.Z &&
            (phase(zxg, v) in (PiUnit(1//2), PiUnit(3//2)))
...
function Base.match(::Rule{:p2}, zxg::ZXGraph{T, P}) where {T, P}
    ...
    for v1 in vs
        if spider_type(zxg, v1) == SpiderType.Z && length(searchsorted(vB, v1)) == 0 &&
            (degree(zxg, v1)) > 1 && !is_clifford(zxg, v1) &&
            length(neighbors(zxg, v1)) > 1 && v1 ∉ v_matched
            for v2 in neighbors(zxg, v1)
                if spider_type(zxg, v2) == SpiderType.Z &&
                    length(searchsorted(vB, v2)) == 0 && is_pauli(zxg, v2)
...
function Base.match(::Rule{:p3}, zxg::ZXGraph{T, P}) where {T, P}
    ...
    for v1 in vB
        if spider_type(zxg, v1) == SpiderType.Z && length(searchsorted(vB, v1)) > 0 &&
            !is_clifford(zxg, v1) && length(neighbors(zxg, v1)) > 1 &&
            v1 ∉ v_matched
            for v2 in neighbors(zxg, v1)
                if spider_type(zxg, v2) == SpiderType.Z && length(searchsorted(vB, v2)) == 0 &&
                    is_pauli(zxg, v2) && length(searchsorted(gadgets, v2)) == 0 && v2 ∉ v_matched
                    push!(matches, Match{T}([v1, v2]))
                    push!(v_matched, v1, v2)
                end
            end
...
function rewrite!(r::Rule{:lc}, zxg::ZXGraph{T, P}, vs::Vector{T}) where {T, P}
    @inbounds v = vs[1]
    phase_v = phase(zxg, v)
    if phase_v == PiUnit(1//2)
        add_global_phase!(zxg, PiUnit(1//4))
    else
        add_global_phase!(zxg, PiUnit(-1//4))
    end
    nb = neighbors(zxg, v)
...
function rewrite!(::Rule{:p1}, zxg::ZXGraph{T, P}, vs::Vector{T}) where {T, P}
    ...
    phase_u = phase(zxg, u)
    phase_v = phase(zxg, v)
    (phase_u == phase_v == one(PiUnit)) && add_global_phase!(zxg, one(PiUnit))
    ...
    for w0 in W
        set_phase!(zxg, w0, phase(zxg, w0)+phase_u+phase_v+one(PiUnit))
    end
    return zxg
end
...
function rewrite!(::Rule{:pab}, zxg::ZXGraph{T, P}, vs::Vector{T}) where {T, P}
    ...
    is_hdm_bd = is_hadamard(zxg, v, v_bound)
    ...
    zxg.et[(v_bound, w)] = is_hdm_bd ? EdgeType.HAD : EdgeType.SIM
...
function rewrite!(::Rule{:p2}, zxg::ZXGraph{T, P}, vs::Vector{T}) where {T, P}
    ...
    sgn_phase_v = (phase_v == zero(PiUnit) ? 1 : -1)
    ...
    for w0 in W
        set_phase!(zxg, w0, phase(zxg, w0)+phase_v+one(PiUnit))
    end
    sgn_phase_v == -1 && (phase_u = -phase_u)
    gad = add_spider!(zxg, SpiderType.Z, P(phase_u))
    add_edge!(zxg, v, gad)
    set_phase!(zxg, v, zero(P))
...
function rewrite!(::Rule{:pivot}, zxg::ZXGraph{T, P}, vs::Vector{T}) where {T, P}
    ...
    for w0 in W
        set_phase!(zxg, w0, phase(zxg, w0)+one(PiUnit))
    end
...
function rewrite!(::Rule{:id}, zxg::ZXGraph{T, P}, vs::Vector{T}) where {T, P}
    ...
    phase_master_id1 = phase(zxg.master, id1)
    phase_master_id3 = phase(zxg.master, id3)
    (mul1 * mul3 == -1) && (phase_master_id1 = -phase_master_id1)
    set_phase!(zxg.master, id3, phase_master_id3 + phase_master_id1)
...
function rewrite!(::Rule{:gf}, zxg::ZXGraph{T, P}, vs::Vector{T}) where {T, P}
    ...
    phase_master_idv = phase(zxg.master,idv)
    phase_master_idu = phase(zxg.master,idu)
    (mulu * mulv == -1) && (phase_master_idu = -phase_master_idu)
    set_phase!(zxg.master, idv, phase_master_idv + phase_master_idu)
