...
is_half_integer_phase(p::AbstractPhase)::Bool = throw(MethodError(is_half_integer_phase, p))
...
is_half_integer_phase(p::Phase) = (p.ex isa Number) && (rem(p.ex, 1, RoundDown) == 1//2)
