...
function add_ancilla!(zxd::ZXDiagram, in_stype::SpiderType.SType, out_stype::SpiderType.SType; 
        force_add_to_inputs::Bool=false, force_add_to_outputs::Bool=false)
    v_in = add_spider!(zxd, in_stype)
    v_out = add_spider!(zxd, out_stype)
    (force_add_to_inputs || in_stype === SpiderType.In) && push!(zxd.inputs, v_in)
    (force_add_to_outputs || out_stype === SpiderType.Out) && push!(zxd.outputs, v_out)
