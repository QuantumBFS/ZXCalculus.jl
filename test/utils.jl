...
using Yao: Y, put, rot, mat, operator_fidelity
...
@testset "More Basic Gates" begin
	zxwd = ZXWDiagram(1)

	theta = rand()
	push_gate!(zxwd, Val(:Y), 1, theta)

	@test abs(tr(to_matrix(zxwd)' * mat(put(1, 1 => rot(Y, theta * π)))) / 2) ≈ 1
end
